<template>
	<view>
		...
		<view class="goods-section">
			...
			<!-- 商品列表 -->
			<view class="g-item" v-for="(item,index) in orderList" :key="item.recId">
		...
		<view class="footer">
			<view class="price-content">
				...
				<text class="price">{{total-35}}</text>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			submit(){
				let url="/shop/v1/shoppingCart/createOrder"
				let data=[];
				...
				let obj={
					buyItemInfoParamList:data,
					remark:"尽快发货"
				}
				console.log(data,555)
				this.$MyUtils.fetch4Resp(url,obj,'post').then(res => {
				     if (res.code == 200) {
